		...
		<?php if ( ! is_user_logged_in() ) { ?>
			...
			<div class="rh_modal__login_wrap modal">
				...
				<form id="rh_modal__login_form" action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="POST" enctype="multipart/form-data">
					<input name="log" class="focus-class" autocomplete="username" id="username" type="text" placeholder="<?php esc_attr_e( 'Username', 'framework' ); ?>" title="<?php esc_attr_e( '* Provide user name!', 'framework' ); ?>" required autofocus />
					<input name="pwd" autocomplete="current-password" id="password" type="password" placeholder="<?php esc_attr_e( 'Password', 'framework' ); ?>" title="<?php esc_attr_e( '* Provide password!', 'framework' ); ?>" required />
					<?php
					if ( class_exists( 'Easy_Real_Estate' ) ) {
						if ( ere_is_reCAPTCHA_configured()
							...
							<?php
						} elseif ( ere_is_reCAPTCHA_configured()
