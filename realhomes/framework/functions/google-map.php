	...
	function inspiry_enqueue_google_maps() {
		if ( inspiry_is_map_needed() ) {
			...
			if ( is_singular( 'property' ) && ( 'true' == get_option( 'theme_display_google_map' ) ) ) {
				...
				wp_enqueue_script(
					'submit-google-map',
					INSPIRY_DIR_URI . '/scripts/js/submit-google-map.js',
...
if( !function_exists( 'inspiry_render_multi_properties_google_map' ) ) :
	...
	function inspiry_render_multi_properties_google_map() {
		...
		wp_register_script( 'properties-google-map', INSPIRY_DIR_URI . '/scripts/js/properties-google-map.js', array( 'jquery', 'google-map-api' ), INSPIRY_THEME_VERSION, true );
		...
		if ( is_page_template( array( 'templates/properties-search.php', 'templates/properties-search-left-sidebar.php', 'templates/properties-search-right-sidebar.php' ) ) ) {
			...
			$properties_map_options['type'] = get_option( 'inspiry_home_map_type', 'roadmap' );
...
if ( ! function_exists( 'inspiry_render_contact_google_map' ) ) :
	...
	function inspiry_render_contact_google_map() {
		...
		wp_register_script( 'contact-google-map', INSPIRY_DIR_URI . '/scripts/js/contact-google-map.js', array( 'jquery', 'google-map-api' ), INSPIRY_THEME_VERSION, true );
		...
		if ( $contact_map_lat && $contact_map_lang ) {
			...
			$map_zoom  = intval( get_option( 'theme_map_zoom' ) );
			if ( 0 < $map_zoom ) {
			...
			$contact_map_data['type'] = get_option( 'inspiry_contact_map_type', 'roadmap' );
...
if ( ! function_exists( 'inspiry_render_property_google_map' ) ) :
	...
	function inspiry_render_property_google_map() {
		...
		wp_register_script( 'property-google-map', INSPIRY_DIR_URI . '/scripts/js/property-google-map.js', array( 'jquery', 'google-map-api' ), INSPIRY_THEME_VERSION, true );
