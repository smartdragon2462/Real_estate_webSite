	...
	function theme_pagination( $pages = '' ) {
		...
		if ( 1 != $pages ) {
			...
			if ( $paged > 2 && $paged > $range + 1 && $show_items < $pages ) {
				echo "<a href='" . get_pagenum_link( 1 ) . "' class='real-btn rh_arrows_left'> " . esc_html__( 'First', 'framework' ) . "</a> ";
			}

			if ( $paged > 1 && $show_items < $pages ) {
				echo "<a href='" . get_pagenum_link( $prev ) . "' class='real-btn rh_arrows_left' > " . esc_html__( 'Previous', 'framework' ) . "</a> ";
			}
			...
			if ( $paged < $pages && $show_items < $pages ) {
				echo "<a href='" . get_pagenum_link( $next ) . "' class='real-btn rh_arrows_right' >" . esc_html__( 'Next', 'framework' ) . "</a> ";
			}

			if ( $paged < $pages - 1 && $paged + $range - 1 < $pages && $show_items < $pages ) {
				echo "<a href='" . get_pagenum_link( $pages ) . "' class='real-btn rh_arrows_right' >" . esc_html__( 'Last', 'framework' ) . "</a> ";
			}
	...
	function inspiry_theme_pagination( $pages = '' ) {
		...
		if ( 1 != $pages ) {
			...
			if ( $paged > 1 && $show_items < $pages ) {
				...
				include( INSPIRY_THEME_DIR . '/images/icons/icon-left.svg' );
				echo "</a>";
			}
			...
			if ( $paged < $pages && $show_items < $pages ) {
				...
				include( INSPIRY_THEME_DIR . '/images/icons/icon-right.svg' );
				echo "</a>";
			}
	...
	function theme_ajax_pagination( $pages = '' ) {
		...
		if ( 1 != $pages ) {
			...
			if ( ( $paged > 2 ) && ( $paged > $range + 1 ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( 1 ) . "' class='real-btn'>&laquo; " . esc_html__( 'First', 'framework' ) . "</a> "; // First Page
			}

			if ( ( $paged > 1 ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( $prev ) . "' class='real-btn'>&laquo; " . esc_html__( 'Previous', 'framework' ) . "</a> "; // Previous Page
			}
			...
			if ( ( $paged < $pages ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( $next ) . "' class='real-btn'>" . esc_html__( 'Next', 'framework' ) . " &raquo;</a> "; // Next Page
			}

			if ( ( $paged < $pages - 1 ) && ( $paged + $range - 1 < $pages ) && ( $pages_to_show < $pages ) ) {
				echo "<a href='" . get_pagenum_link( $pages ) . "' class='real-btn'>" . esc_html__( 'Last', 'framework' ) . " &raquo;</a> "; // Last Page
			}
